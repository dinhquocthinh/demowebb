...
class PDFRefundTable extends PDFFieldBase
{
    ...
    protected function GetTaxes()
    {
        ...
        if ( 'itemized' === get_option( 'woocommerce_tax_total_display' ) ) {
            ...
            foreach($this->GetOrders() as $order)
                foreach($order->get_taxes() as $tax){
                    ...
                    $taxes[]=array(
                        ...
                        'value'=>wc_price(($this->MaybeAdjustPriceSign((float) $tax->get_tax_total() + (float) $tax->get_shipping_tax_total())),array( 'currency' => $this->orderValueRetriever->get('currency') ))
                    );
            ...
            $taxes[]=array(
                ...
                'value'=>wc_price($this->MaybeAdjustPriceSign($total), \apply_filters('rnwcinv_format_price',array( 'currency' =>  $this->orderValueRetriever->get('currency')) ) )
            );
        }
        return $taxes;
    }
    ...
    protected function GetTotalValue($totalType,$omitIfEmpty,$showInNegativeNumber)
    {
        if($totalType=='quantity')
        {
            ...
            return $this->OrderToUse->get_total_qty_refunded()*-1;
        }
        ...
        if($totalType=='shipping')
        {
            ...
            $price= \wc_price($priceToUse,\apply_filters('rnwcinv_fornmat_price',array( 'currency' =>  $this->orderValueRetriever->get('currency')) ));
        ...
        if($totalType=='subtotal')
        {
            ...
            return wc_price($this->MaybeAdjustPriceSign($total), \apply_filters('rnwcinv_format_price',array( 'currency' => $this->orderValueRetriever->get('currency') )));
        }
        ...
        foreach ($this->OrderToUse->get_items() as $item)
        {
            ...
            $discount=0;
                ...
                $newItem = array(
                    ...
                    'prod' => $item['name'],
                    'qty' => $qty,
                    'price' => wc_price($price, \apply_filters('rnwcinv_format_price', array('currency' => $this->orderValueRetriever->get('currency')))),
                    'vat' => wc_price($this->IsFullRefund()?$item['line_tax']:$this->GetTotalRefundedByItem($item->get_id()), \apply_filters('rnwcinv_format_price', array('currency' => $this->orderValueRetriever->get('currency')))),
                    'discount' => 0,
                    'sku' => $sku,
                    ...
                    'total' => wc_price($price, \apply_filters('rnwcinv_format_price', array('currency' => $this->orderValueRetriever->get('currency'))))
                );
